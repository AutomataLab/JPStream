...
#define JSONQueryDFA_ARRAY ((const char*)0)

#define JSONQueryDFA_OUTPUT_TYPE (1)
#define JSONQueryDFA_PREDICATE_TYPE (2)
...
static inline uint32_t getDFAInputsNum(JSONQueryDFA* dfa) {
...
static inline int32_t dfaNextStateByStr(JSONQueryDFA* dfa, uint32_t state, const char* input) {
    ...
    if (input == NULL) cinput = 2;
        ...
        for (cinput = 3; cinput < getDFAInputsNum(dfa); ++cinput) {
...
static inline void printDFA(JSONQueryDFA* dfa) {
    ...
    uint32_t inputs = getDFAInputsNum(dfa);
