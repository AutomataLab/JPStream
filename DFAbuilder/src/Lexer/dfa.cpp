...
int DFA::nextState(int s, regex_char ch) {
...
void DFA::addEdge(int s, const std::vector<int>& objvec, int max) {
    // cout << "addEdge: " << s << endl;
    // for (auto p : objvec) {
    //     cout << p << ' ';
    // }
...
void DFA::createNewArea(int base, int s, const std::vector<int>& objvec) {
    m_default[s] = -base;
...
void DFA::createLinkArea(int base, int s, const std::vector<int>& objvec) {
    m_default[s] = base;
    for (int i = Top; i <= Bottom; ++i) {
        // first check the space 
        bool is_first = true; 
